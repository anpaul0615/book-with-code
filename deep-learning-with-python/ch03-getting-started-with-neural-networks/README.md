...
- 보스턴 주택 가격 데이터세트 :  
  훈련데이터 404개, 테스트데이터 102개 (주택 관련 특성수치 벡터)  
  각 데이터포인트에 대한 타깃 (주택 가격, 1000달러단위) 

- 각 데이터샘플은 1인당 범죄율, 평균 방의 개수, 고속도로 접근성 등의 특성수치들로 구성되어 있음.

- 입력데이터의 특성이 서로 다른 범위를 가지면, 전처리 단계에서 각 특성을 개별적으로 스케일 조정해야함.  
  => 상이한 스케일을 가진 값을 신경망에 주입하면 안되기때문임  
  => 훈련데이터의 각 특성별로 정규화(표준화) 진행  
  => 데이터의 각 특성(입력데이터 행렬의 열) 에 대해서 특성의 평균을 빼고 표준편차로 나눠줌  
  => 특성의 중앙이 0 근처에 맞추어지고 표준편차가 1 로 통일됨

- 훈련데이터를 정규화할때 훈련데이터에서 계산된 평균과 표준편차를 사용함.  
  => 테스트데이터를 정규화할때도 훈련데이터에서 계산된 값들을 사용해야함  
  => 머신러닝 작업과정에서는 절대로 테스트데이터를 건드리면 안됨

- 일반적으로 훈련데이터의 개수가 적을수록 과대적합이 더 쉽게 일어남.  
  => 작은 모델을 사용하는 것이 과대적합을 피할수있는 방법이 될수있음

- 네트워크의 마지막층이 하나의 유닛을 가지고있고 활성화함수가 없다..?  
  => 전형적인 스칼라 회귀(하나의 연속적인 값을 예측하는 회귀) 를 위한 네트워크 구성임!  

- 활성화함수가 없는 층을 선형층 이라고 함.  
  => 선형층은 네트워크가 어떤 범위의 값이라도 예측하도록 자유롭게 동작하게 함

- 일반적으로 활성화함수를 사용하면 출력값의 범위를 제한하게 됨.  
  => 예를들어 sigmoid 활성화함수는 네트워크가 0~1 사이의 값을 예측하도록 동작하게 함

- mse(mean squared error, 평균제곱오차) 손실함수..?  
  => 예측과 타깃 사이 거리의 제곱  
  => 회귀문제에서 널리 사용되는 손실함수

- MAE(Mean Absolute Error, 평균절대오차) 지표..?  
  => 예측과 타깃 사이 거리의 절대값  
  => 훈련 결과를 모니터링하기 위해 사용되는 지표

- K-겹 교차 검증(K-fold cross-validation) 이란..?  
  => 데이터를 K 개의 분할로 나누고, K 개의 모델을 각각 만들어서, K-1 개의 분할에서 훈련하고 나머지 분할에서 평가하는 방법  
  => 데이터가 많지않을때 사용하는 모델 평가 방법

- K개의 분할에서도 훈련데이터와 검증데이터의 스케일을 표준화할때가있음.  
  => 이때도 역시 훈련용 분할에서의 평균/표준편차를 사용해서 검증용 분할을 표준화해야함

- 지수이동평균(exponential moving average, EMA) 이란..?  
  => 최근 데이터에 더 높은 가중치를 두는 일종의 가중 이동평균법  
  => 최근 시점에 비중을 크게 주기때문에 시장분위기를 잘 반영함 + 오래된 데이터를 갑자기 사라지게 하지않고 천천히 그 영향력을 약화시킴



## 3.7. 요약

- 보통 입력데이터를 신경망에 주입하기전에 전처리를 진행함.

- 데이터에 범위가 다른 특성이 있다면, 전처리단계에서 각 특성을 독립적으로 스케일 조정해야 함.

- 훈련이 진행됨에 따라 신경망의 과대적합이 시작됨. => 새로운 데이터에 대해 나쁜 결과를 얻게됨.

- 훈련데이터가 많지 않으면, 은닉층의 수를 적게하는것으로 과대적합을 피할수있음.

- 데이터가 많은 범주로 나뉘어있을때, 중간층이 너무 적으면 정보의 병목이 생길수있음.

- 적은 데이터를 사용할때는, K-fold 교차검증법을 사용해서 모델 평가의 신뢰성을 높일수있음.

- 회귀는 분류와 다른 손실함수와 평가지표를 사용함.
