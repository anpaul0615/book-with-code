 ...
 * Step 50
 * 
 * - 테스트 대상코드 수정 (가산수,피가산수 를 Expression 으로 취급)
 * - 테스트 통과 확인
 */
namespace step50 {
  ...
  class Money implements Expression {
    ...
    // plus(target: Money) {
    plus(target: Expression): Expression {  // 가산수,피가산수 를 Expression 으로 취급
  ...
  class Sum implements Expression {
    ...
    constructor(augend: Expression, addend: Expression) {  // 가산수,피가산수 를 Expression 으로 취급
      this.augend = augend;
      this.addend = addend;
    }
    reduce(bank: Bank, currency: string): Money {
   ...
   * Test Suites
   */
  describe.skip('Differnct Currency Calculation (Step 50)', ()=>{
