 ...
 * Step 48
 * 
 * - 테스트 실행코드 수정 (Money.dollar, Money.franc 값을 Money 로 취급)
 * - 테스트 실패 확인 (currency 계산 없이 단순하게 amount 를 합산함)
 */
namespace step48 {
   ...
   * Test Suites
   */
  describe.skip('Differnct Currency Calculation (Step 48)', ()=>{
    test('5USD + 10CHF = 10USD Test', ()=>{
      const five_dollars: Money = Money.dollar(5);  // Money.dollar, Money.franc 값을 Money 로 취급
      const ten_francs: Money = Money.franc(10);    // 상동
      const bank: Bank = new Bank();
      bank.addRate('CHF', 'USD', 2);
      const result: Money
        = bank.reduce( five_dollars.plus(ten_francs), 'USD' );
      expect( result ).toEqual( Money.dollar(10) );  // 환율 계산없이 amount 끼리만 더함 (5 + 10 = 15 로 계산됨)
    });
  });
