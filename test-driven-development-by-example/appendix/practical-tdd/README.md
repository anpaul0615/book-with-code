...
> 데이터베이스 프로그래밍을 할 때도 전반적인 접근 방법은 앞의 방법들과 비슷하다.

> (도서 관리 프로그램 구현 목록)  
> - 책 정보를 서가(책 저장소)에 저장하기  
> - 책 목록을 서가에서 가져오기  
> - 책 정보를 서가에서 가져오기  
> - 책 정보를 서가에서 검색하기  

> 이에 대해서도 역시 로직부터 TDD 로 작성할 수 있다. 책 정보를 DB 에 저장한다고 구체적으로 표헌하지 않았다. 서가가 "메모리 저장소" 면 된다. 글을 가져온 곳이 메모리건 파일이건 데이터베이스건 결국은 저장소인 것이다.


### 데이터베이스에 대한 모의 객체

> DB 에 대해 TDD 를 진행할 경우 DB 세팅이 필요하다. DB 세팅은 시간이걸리고 임시 데이터를 채워 넣어야 하므로 준비할 작업이 많다. DB 세팅 없이도 바로 TDD 를 할 수 있는 방법으로는 간단하게 모의 DB 커넥션 객체를 만드는 방법이 있다.

> 모의 객체를 이용할 떄는 두 가지 테스트가 같이 필요하다. 모의 객체는 말 그대로 모의 객체이기 때문이다. 모의 객체에서는 테스트를 잘 통과하는데, 실제 객체를 이용했을 때는 테스트를 통과하지 않을 수도 있다.

> (모의 객체 기반 테스트의 평가 항목)  
> - 둘 다 성공 : 잘 된 상태.  
> - 모의 객체 성공 / 실제 객체 실패 : 실제 객체의 이용 상태를 확인해야 한다. 테스트 실행 완료 후 데이터베이스의 상태가 제대로 복구되지 않는 등의 문제 상황일 수 있다.  
> - 모의 객체 실패 / 실제 객체 성공 : 모의 객체가 완전히 구현되지 않은 상황이다. 하지만 모의 객체는 어디까지나 보조 도구이므로 구현하는데 해야 하는 일이 너무 많다면 무시해도 무방하다.  
> - 둘 다 실패 : 제대로 구현되지 않은 상황이다. 일단은 실제 객체 테스트를 주석 처리한 뒤, 모의 객체 테스트부터 차근차근 통과시키자.  


## 실제 DB 데이터를 이용하기

> 디자인이 잘 됐다면, 모의 객체 에서의 테스트를 상속받아서 setUp() 내부의 모의 객체를 실제 객체로 바꿔주기만 하면 앞의 테스트들을 거의 그대로 돌릴 수 있을 것이다.

> 실제 DB 데이터를 이용할 때는 테스트를 위한 DB 를 미리 준비하는 것이 좋다. 또한 TDD 로 DB 프로그래밍을 한다면 DB 를 세 단계로 나누는 것이 좋다. 하나는 실제 DB, 하나는 통합 테스트용 DB, 하나는 개발용 로컬 DB 다.

> 실제 DB 를 이용한 테스트가 많으면 많을수록 테스트 실행 속도가 많이 느려진다. 테스트는 TDD 가 진행될 때마다 실행되므로 느려지면 상당히 피곤해진다. 이 경우 모의 객체를 같이 섞어서 프로그래밍하는 것이 좋겠다.

> 그리고 실수하기 쉬운 것 중 하나가 DB 테스트 후 DB 환경을 원상 복구하지 않고 TDD 를 진행하기도 하는데, 그런 경우 처음 테스트는 잘 통과한 코드가 나중에 가서는 실패할 수도 있다. DB 테스트에서는 setUp/tearDown 부분 등에서 DB 상태를 원상 복구 하는것을 잊으면 안된다.



## 꾸준한 수련이 중요

> 모듈 내 작은 단위테스트를 작성해 내가며 TDD 로 작업하는 것을 같이 하도록 권장한다. TDD 에서의 테스팅은 프로그램의 동작을 테스트하는 목적뿐만 아니라, 디자인의 목적도 같이 있기 때문에 가볍고 유연한 테스팅 프레임워크를 이용하는 것이 좋다.

> TDD 를 제대로 사용하기 위해서는 반 년 이상 꾸준히 노력해야 한다고 생각한다. 그 기간을 줄이는 방법이 있다면 짝 프로그래밍, 팀과 함께 작업하기 등 익스트림 프로그래밍의 실천사항들을 병행하는 것이 아닐까 한다. 그래도 역시 TDD 를 꾸준히 하는 것이 가장 중요하겠지만 말이다.

> 주변에서 꾸준히 TDD 를 하는 사람에게서 나날이 성장하는 모습을 발견할 수 있었다. 우리 역시 TDD 를 통해 많은 것을 빠르게 배울 수 있었다. 그리고 프로그래밍이 더욱 즐겁고 행복해졌다. 이 복음을 다른 프로그래머들과 함께 공유했으면 하느 바람이다.
